...
def config():
    return {
        "file_path": str(FIXTURES / "sample_cameras.csv"),
        "column_mapping": {
            "lat": "lat",
            "lon": "lon",
            "type": "type",
...
async def test_fetch_extracts_external_id(adapter, config):
    records = await adapter.fetch(config)
    assert records[0].external_id == "CSV-001"


async def test_fetch_handles_missing_coords(adapter, config):
    records = await adapter.fetch(config)
    # Row 4 has no lat/lon
    assert records[3].lat is None
    assert records[3].lon is None
    assert records[3].external_id == "CSV-004-no-coords"
...
async def test_fetch_custom_column_mapping(adapter):
    config = {
        "file_path": str(FIXTURES / "sample_cameras_custom.csv"),
        "column_mapping": {
            "lat": "latitude",
            "lon": "longitude",
            "type": "camera_type",
...
async def test_fetch_raises_on_missing_file(adapter):
    config = {"file_path": "/nonexistent/file.csv", "column_mapping": {"lat": "lat", "lon": "lon"}}
...
async def test_fetch_handles_empty_file(adapter):
    config = {
        "file_path": str(FIXTURES / "empty_cameras.csv"),
